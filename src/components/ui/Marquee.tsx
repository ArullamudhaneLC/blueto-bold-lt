...
const Marquee = ({ items, label }: Props) => (
  <div className="-rotate-1 overflow-hidden bg-brand-600 py-4 text-white" aria-label={label}>
      ...
      {[0, 1].map((copy) => (
        <ul key={copy} className="flex shrink-0 items-center" aria-hidden={copy === 1}>
          {items.map((item) => (
            <li
              key={item}
              className="flex items-center font-display text-2xl font-extrabold tracking-tight sm:text-3xl"
            >
